...
FROM [dbo].[ProcessOperation] 
INNER JOIN [dbo].[Item] 
ON [dbo].[ProcessOperation].ProcessId=[dbo].[Item].RawMaterialCategoryId
INNER JOIN 
[Relate].[PurchaseOrderItemMapping]
ON [dbo].[Item].ITEM_ID=[Relate].[PurchaseOrderItemMapping].ItemId
Where [dbo].[ProcessOperation].OperationId =@Operation and [Relate].[PurchaseOrderItemMapping].PoId = @POId
GO