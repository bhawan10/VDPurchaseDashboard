CREATE PROCEDURE getOperationsFromPO @PONumber nvarchar(50)
AS
...
where [FalconTest].[Relate].[PurchaseOrderItemMapping].POId =(select Distinct PkId 
from [Master].[PurchaseOrder] with (NOLOCK)
where PONumber=@PONumber )) s
